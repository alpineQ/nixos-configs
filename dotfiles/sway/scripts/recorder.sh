...
notify() {
    line=$1
    shift
    notify-send "Recording" "${line}" -i /usr/share/icons/AdwaitaLegacy/32x32/legacy/camera-video.png $*
}
...
if [ $status != 0 ]; then
    ...
    if [ "$1" = "-a" ]; then
        ...
        wf-recorder -g "$area" -c libvpx --codec-param="qmin=0" --codec-param="qmax=25" --codec-param="crf=4" --codec-param="crf=4" --file="$file"
    fi
